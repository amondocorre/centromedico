...
class MedicalController extends CI_Controller {
  ...
  public function delete($id) {
    ...
    $user = $res->user;
    $idUsuario = $user->id_usuario;
    if ($this->MedicalModel->delete($id,$idUsuario)) {
      $url = getHttpHost();
      $res = $this->MedicalModel->findIdentity($id); 
      $res->foto = $res->foto? $url.$res->foto:'';
      $res->nombre_completo = $res->nombre.' '.$res->ap_paterno.' '.$res->ap_materno;
      $response = ['status' => 'success','message'=>'Se ha eliminado correctamente la evaluación.','data'=>$res];
      return _send_json_response($this, 200, $response);
    } else {
      $response = ['status' => 'error', 'message' =>  ['Ocurrió un error al eliminar.']];
      return _send_json_response($this, 400, $response);
    }
  }
