...
class VisualController extends CI_Controller {
  ...
  public function create() {
    ...
    $id = $this->VisualModel->create($data, $idUsuario);
    if($id) {
      if($file){
        $url = guardarArchivo($id, $file, 'assets/evaluacion_visual/');
        if(!$url){
          $response = ['status' => 'success', 'message' => 'Ocurrió un error al guardar la foto.'];
          return _send_json_response($this, 200, $response);
        }
        $this->VisualModel->updateFoto($url, $id);
      } 
      $url = getHttpHost();
      $res = $this->VisualModel->findIdentity($id); 
  ...
  public function update($id) {
    ...
    if ($this->VisualModel->update($id, $data, $idUsuario)) {
      if($file){
        $url = guardarArchivo($id, $file, 'assets/evaluacion_visual/');
        if(!$url){
          $response = ['status' => 'success', 'message' => 'Ocurrió un error al guardar la foto.'];
  ...
  public function activate($id) {
    ...
    $user = $res->user;
    $idUsuario = $user->id_usuario;
    if ($this->VisualModel->activate($id, $idUsuario)) {
      $url = getHttpHost();
      $res = $this->VisualModel->findIdentity($id); 
      $res->foto = $res->foto ? $url.$res->foto : '';
      $res->nombre_completo = $res->nombre.' '.$res->ap_paterno.' '.$res->ap_materno;
  ...
  public function search() {
      ...
      $data = $this->VisualModel->search($q);
